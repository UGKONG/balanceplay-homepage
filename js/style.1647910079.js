var style = new function () {
		...
		switch(k) {
			...
			case 'background-image':
				if(base.selector.selector.indexOf('galProjectBg')>-1) {
					...
					console.log(base.property.bgUrl);
	...
	this.getAttributesMoreBtnColor = function(j,selector,key,hover) {
		...
		var hover_check = (selector.indexOf(':hover') > -1) ? true : false,
			selector_ori = selector;
			if(hover) selector = selector+':hover';
		var color_val = '';
				...
				if(!color_val) {
					...
					if(border_css) {
						...
						$.each(border_arry, function(i,v){
							if(v.indexOf('#') > -1 || v.indexOf('transparent') > -1) color_val = v;
						});
					}
				}
